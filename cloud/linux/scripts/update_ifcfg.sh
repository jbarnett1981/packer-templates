...
if grep -q PEERDNS "$IFCFG"; then sed -i 's/^PEERDNS.*/PEERDNS=no/' $IFCFG; else bash -c 'echo "PEERDNS=no" >> $IFCFG'; fi

# Add correct HWADDR to ifcfg-eth0
if grep -q HWADDR "$IFCFG"; then sed -i "s/^HWADDR.*/HWADDR=${HWADDR}/" $IFCFG; else bash -c 'echo "HWADDR=${HWADDR}" >> $IFCFG'; fi
